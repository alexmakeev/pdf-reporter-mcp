...
import { generatePalette } from '../src/color-utils.js';
...
async function main() {
  ...
  const archDiagram = await renderDiagram({
    ...
    style A fill:#DBEAFE,color:#1a3a6e,stroke:#4169E1
    style B fill:#E0E7FA,color:#2b3a6e,stroke:#4169E1
    style C fill:#DCEDC8,color:#2d5016,stroke:#558B2F
    style D fill:#F3E5F5,color:#4a1462,stroke:#7B1FA2
    style E fill:#FFF9C4,color:#6b5d00,stroke:#F9A825
    style F fill:#FFF9C4,color:#6b5d00,stroke:#F9A825
    style G fill:#C8E6C9,color:#1b5e20,stroke:#388E3C`,
  });
  ...
  const flowDiagram = await renderDiagram({
    name: 'pipeline',
    mermaid: `%%{init: {'theme': 'base', 'themeVariables': {'actorBkg': '#DBEAFE', 'actorTextColor': '#1a3a6e', 'actorBorder': '#4169E1', 'signalColor': '#4169E1', 'signalTextColor': '#1a3a6e', 'noteBkgColor': '#FFF9C4', 'noteTextColor': '#6b5d00', 'activationBkgColor': '#E0E7FA', 'sequenceNumberColor': '#1a3a6e'}}}%%
sequenceDiagram
    participant Client
  ...
  const pieDiagram = await renderDiagram({
    name: 'stats',
    mermaid: `%%{init: {'theme': 'base', 'themeVariables': {'pie1': '#DBEAFE', 'pie2': '#DCEDC8', 'pie3': '#F3E5F5', 'pie4': '#FFF9C4', 'pieStrokeColor': '#8899aa', 'pieSectionTextColor': '#2a2a3a', 'pieTitleTextColor': '#1a3a6e', 'pieLegendTextColor': '#2a2a3a'}}}%%
